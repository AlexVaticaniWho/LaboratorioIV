...
int main(int argc, char *argv[]) {
  ...
  unsigned char buf[4096], sht75_nblab03_frame[4], sht75_nblab03_control[4]; // buf è un vettore di 4096 byte (char) organizzati in char
       ...
       // buf[n] = 0;   // l'(n+1)esimo byte lo poniamo nullo quando stiamo acquisendo i primi n
        ...
        for (i = 1; i < n; i++) { 
          ...
          //quando i=0 e i=1, gira a vuoto perché initFlag=0
					if (buf[i+ 5*k] == 0xAA && buf[i - 1 + 5*k] == 0xAA) {          
            StartFlag = 0;
            nloc = 0;
          ...
          if (InitFlag == 1) { //a questo punto abbiamo nloc=0 e i=2
