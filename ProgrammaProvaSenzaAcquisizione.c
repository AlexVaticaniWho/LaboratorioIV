...
int main(int argc, char *argv[])
{
  ...
  nloc = 0;
  int InitFlag, StartFlag, n, nhit, hit, trg = 0, nresto, i, k = 0, rebuildPackages = 0;
  ...
  { // itero per un numero infinito di volte (condizione sempre verificata)
    ...
    else
    {
      ...
      if (n > 0)
      {
        ...
        for (i = 0; i < n; i++)
        {
          // i va da 0 a n-1, con n la lunghezza dei byte presi. Per ogni i avremo Misurazione_nloc = buf_i
          // quando i = 0 e i = 1, gira a vuoto perché initFlag=0
          /* va richiesto i > 0 perché altrimenti viene valutato anche l'elemento buf[-1] che, per qualche motivo, non da errore di segmentazione ma
          ...
          if (InitFlag == 1)
          {
            ...
            misurazioni[nloc] = buf[i]; // a partire dal terzo byte di buf riempie il primo di misurazioni corrispondente a nloc=0
            printf("\n nloc = %d \n i = %d \n", nloc, i);
            fflush(stdout);
            ...
            else if (nloc == 3)
            {
              ...
              printf("Pacchetto %d\n%3x%3x\n%3x%3x\n%3x%3x\n\n", trg, buf[i], buf[i + 1], buf[i + 2], buf[i + 3], buf[i + 4], buf[i + 5]);
              fflush(stdout);
              if (cnt % 100 == 0)
              {
