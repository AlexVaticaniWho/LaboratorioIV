...
int acquisizione(int n, int *nloc, bool rebuildPackages);
...
int main(int argc, char *argv[])
{
  ...
  unsigned char buf[4096], misurazioni[4], sht75_nblab03_control[4]; // buf è un vettore di 4096 byte (char) organizzati in char
  ...
  { // itero per un numero infinito di volte (condizione sempre verificata)
    ...
    else
    {
      ...
      {
        if (lostedInfo > 0)
        { // Caso in cui ho perso dati nella precedente iterazione
          acquisizione(6 - lostedInfo, (&nloc) + 1, true);
        }

        lostedInfo = acquisizione(n, (&nloc) + 1, false);
      }
...
int acquisizione(int n, int *nloc, bool rebuildPackages)
{
  ...
  else
  {
    ...
    { // Se lostedInfo è 0 allora pongo index = 0 perché non ho informazioni perse nella precedente acquisizione (6 - losted info sarebbe stato 6 e avrei perso un pacchetto)
      index = 0;
    }
  }
  ...
  for (index; i < n; i++)
  {
    ...
    *nloc++; // faccio crescere ad ogni iterazione nloc che tiene conto delle coppie di bytes acquisite
    ...
    if (nresto != 0)
    {
      informazioniPerse = nresto;
    }
  }
  return informazioniPerse;
}