...
int main(int argc, char *argv[]){
  ...
  int cnt=0;
  FILE *file;
  FILE *currN; //file che salva il nome del file corrente affinchè possa essere usato da altri programmi
  ...
  if (argv[1] == NULL)
       ...
       /***scrivo il nome del file nel file nome corrente affinchè possa essere usato da programmi esterni***/
  ...
  while(1){     //itero per un numero infinito di volte (condizione sempre verificata)
    ...
    if(n > 0){
       ...
       buf[n] = 0; // l'(n+1)esimo byte è nullo quando stiamo acquisendo i primi n
       ...
       for(i=0; i < n; i++){
	    ...
	    if (InitFlag==1){ //parte dal terzo byte, dopo il primo di controllo
	        ...
	        if (nloc==1){
              ...
              val_hum_int=(sht75_nblab03_frame[0]<<8)|(sht75_nblab03_frame[1]); /*OR (restituisce un int) tra il primo byte traslato e il secondo;
                                                                                in questo modo mettiamo in sequenza i due byte di umidità relativo e li leggiamo
        ...
        nloc++; //faccio crescere ad ogni iterazione nloc che tiene conto delle coppie di bytes acquisite
         ...
         if (i>0 && buf[i]==0xAA && buf[i-1]==0xAA) //&& se entrambi gli operatori sono diversi da zero (veri) è una condizione vera
